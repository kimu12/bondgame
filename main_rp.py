...
ion_sprites_group = Ion_group() # pygame.sprite.Group()
# ion_sprites_list = []
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1: #'1' - Is left mousebutton being clicked?
                ...
                for i, puzzle_piece in enumerate(ion_sprites_group):
                    ...
                    #print("RP..", puzzle_piece.rect, puzzle_piece.sprite_width, puzzle_piece.sprite_height)
                    #print("dimensions:", puzzle_piece.rect[0], puzzle_piece.rect[0] + puzzle_piece.sprite_width, puzzle_piece.rect[1], puzzle_piece.rect[1] + puzzle_piece.sprite_height)
                    #print("position:", event.pos[0], event.pos[1])
                    if puzzle_piece.rect[0] <= event.pos[0] <= puzzle_piece.rect[0] + puzzle_piece.sprite_width and puzzle_piece.rect[1] <= event.pos[1] <= puzzle_piece.rect[1] + puzzle_piece.sprite_height:
            ...
            if selected is not None:  # selected can be `0` so `is not None` is required
                ...
                for i, puzzle_piece in enumerate(ion_sprites_group):
                    if i == selected:
                        ...
                        puzzle_piece.pos_x = event.pos[0] + selected_offset_x
                        puzzle_piece.pos_y = event.pos[1] + selected_offset_y
